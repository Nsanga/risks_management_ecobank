    ...
    return (
      <ListItem>
        <Box display="flex" alignItems="center">
          ...
          <Text ml={level * 4} onClick={!hasSubCategories ? () => onItemClick(category) : undefined}>
            {category}
          </Text>
        </Box>
        {hasSubCategories && (
            ...
            <List pl={6} mt={1}>
              {Array.isArray(subCategories)
                ? subCategories.map((item, index) => (
                  <ListItem key={`${item}-${index}`} onClick={() => onItemClick(item)} cursor='pointer' _hover={{ backgroundColor: 'gray.200' }} pl={8}>
                    {item}
                  </ListItem>
                ))
    ...
    <Box p={4}>
      <List spacing={3}>
        ...
        {data.data.types && data.data.types.map((type, index) => (
          <ListItem key={index} onClick={() => onItemClick(type)} cursor='pointer' _hover={{ backgroundColor: 'gray.200' }} >
            {type}
          </ListItem>
        ))}
        {data.data.classifications && data.data.classifications.map((classification, index) => (
          <ListItem key={index} onClick={() => onItemClick(classification)} cursor='pointer' _hover={{ backgroundColor: 'gray.200' }} >
            {classification}
          </ListItem>
        ))}
      </List>
